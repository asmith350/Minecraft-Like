...
public class Chunk {
    ...
    public void UpdateChunk() {
        ...
        {
            // removes first voxelmod from queue (FIFO)
    ...
    void UpdateSurroundingVoxels(int _x, int _y, int _z)
    {
        Vector3 thisVoxel = new Vector3(_x, _y, _z);
    ...
    void UpdateMeshData(Vector3 pos) {
        ...
        for (int p = 0; p < 6; p++)
        {
            if (CheckVoxel(pos + VoxelData.faceChecks[p])) {
                ...
                if (!isTransparent)
                {
                    // handles overlapping vertices in triangles
    ...
    void AddTexture(int textureID) {
        float y = textureID / VoxelData.TextureAtlasSizeToBlocks;
        float x = textureID - (y * VoxelData.TextureAtlasSizeToBlocks);
